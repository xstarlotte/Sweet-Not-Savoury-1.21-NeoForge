...
public class ParfaitPixieEntity extends TamableAnimal implements GeoEntity {

    private static final EntityDataAccessor<Integer> VARIANT =
            SynchedEntityData.defineId(ParfaitPixieEntity.class, EntityDataSerializers.INT);
    private static final EntityDataAccessor<Boolean> SITTING =
            SynchedEntityData.defineId(ParfaitPixieEntity.class, EntityDataSerializers.BOOLEAN);

    //animations

    public ParfaitPixieEntity(EntityType<? extends ParfaitPixieEntity> type, Level level) {
        super(type, level);
    }
    @Override
    ...
    private <T extends GeoAnimatable> PlayState predicate(AnimationState<T> tAnimationState) {
        if(tAnimationState.isMoving()) {
            tAnimationState.getController().setAnimation(RawAnimation.begin().then("animation.parfait_pixie.fly", Animation.LoopType.LOOP));
            return PlayState.CONTINUE;
        }
        if(this.isSitting()) {
            tAnimationState.getController().setAnimation(RawAnimation.begin().then("animation.parfait_pixie.sit", Animation.LoopType.LOOP));
            return PlayState.CONTINUE;
        }
        tAnimationState.getController().setAnimation(RawAnimation.begin().then("animation.parfait_pixie.idle", Animation.LoopType.LOOP));
        return PlayState.CONTINUE;
    }
    @Override
    ...
    public AgeableMob getBreedOffspring(ServerLevel level, AgeableMob otherParent) {
        return SNSEntity.PARFAIT_PIXIE.get().create(level);
    }
    ...
    public InteractionResult mobInteract(Player pPlayer, InteractionHand pHand) {
        ...
        Item itemForTaming = Items.MILK_BUCKET;
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor pLevel, DifficultyInstance pDifficulty, MobSpawnType pReason,
        ...
        ParfaitPixieVariant variant = Util.getRandom(ParfaitPixieVariant.values(), this.random);
    ...
    public ParfaitPixieVariant getVariant() {
        return ParfaitPixieVariant.byId(this.getTypeVariant() & 255);
    }
    private void setVariant(ParfaitPixieVariant variant) {
