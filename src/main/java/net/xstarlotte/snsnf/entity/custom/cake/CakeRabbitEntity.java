package net.xstarlotte.snsnf.entity.custom.cake;
...
public class CakeRabbitEntity extends TamableAnimal implements GeoEntity {

    private static final EntityDataAccessor<Integer> VARIANT =
            SynchedEntityData.defineId(CakeRabbitEntity.class, EntityDataSerializers.INT);
    private static final EntityDataAccessor<Boolean> SITTING =
            SynchedEntityData.defineId(CakeRabbitEntity.class, EntityDataSerializers.BOOLEAN);

    //animations

    public CakeRabbitEntity(EntityType<? extends CakeRabbitEntity> type, Level level) {
        super(type, level);
    }
    @Override
    ...
    private <T extends GeoAnimatable> PlayState predicate(AnimationState<T> tAnimationState) {
        if(tAnimationState.isMoving()) {
            tAnimationState.getController().setAnimation(RawAnimation.begin().then("animation.cake_rabbit.run", Animation.LoopType.LOOP));
            return PlayState.CONTINUE;
        }
        if(this.isSitting()) {
            tAnimationState.getController().setAnimation(RawAnimation.begin().then("animation.cake_rabbit.sit", Animation.LoopType.LOOP));
            return PlayState.CONTINUE;
        }
        tAnimationState.getController().setAnimation(RawAnimation.begin().then("animation.cake_rabbit.idle", Animation.LoopType.LOOP));
        return PlayState.CONTINUE;
    }
    @Override
    ...
    public AgeableMob getBreedOffspring(ServerLevel level, AgeableMob otherParent) {
        return SNSEntity.CAKE_RABBIT.get().create(level);
    }
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor pLevel, DifficultyInstance pDifficulty, MobSpawnType pReason,
        ...
        CakeRabbitVariant variant = Util.getRandom(CakeRabbitVariant.values(), this.random);
    ...
    public CakeRabbitVariant getVariant() {
        return CakeRabbitVariant.byId(this.getTypeVariant() & 255);
    }
    private void setVariant(CakeRabbitVariant variant) {
