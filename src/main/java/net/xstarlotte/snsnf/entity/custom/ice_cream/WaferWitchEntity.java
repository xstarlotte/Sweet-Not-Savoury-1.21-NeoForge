...
public class WaferWitchEntity extends TamableAnimal implements GeoEntity {

    private static final EntityDataAccessor<Integer> VARIANT =
            SynchedEntityData.defineId(WaferWitchEntity.class, EntityDataSerializers.INT);
    private static final EntityDataAccessor<Boolean> SITTING =
            SynchedEntityData.defineId(WaferWitchEntity.class, EntityDataSerializers.BOOLEAN);

    //animations

    public WaferWitchEntity(EntityType<? extends WaferWitchEntity> type, Level level) {
        super(type, level);
    }
    @Override
    ...
    private <T extends GeoAnimatable> PlayState predicate(AnimationState<T> tAnimationState) {
        if(tAnimationState.isMoving()) {
            tAnimationState.getController().setAnimation(RawAnimation.begin().then("animation.wafer_witch.fly", Animation.LoopType.LOOP));
            return PlayState.CONTINUE;
        }
        if(this.isSitting()) {
            tAnimationState.getController().setAnimation(RawAnimation.begin().then("animation.wafer_witch.sit", Animation.LoopType.LOOP));
            return PlayState.CONTINUE;
        }
        tAnimationState.getController().setAnimation(RawAnimation.begin().then("animation.wafer_witch.idle", Animation.LoopType.LOOP));
        return PlayState.CONTINUE;
    }
    @Override
    ...
    public AgeableMob getBreedOffspring(ServerLevel level, AgeableMob otherParent) {
        return SNSEntity.WAFER_WITCH.get().create(level);
    }
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor pLevel, DifficultyInstance pDifficulty, MobSpawnType pReason,
        ...
        WaferWitchVariant variant = Util.getRandom(WaferWitchVariant.values(), this.random);
    ...
    public WaferWitchVariant getVariant() {
        return WaferWitchVariant.byId(this.getTypeVariant() & 255);
    }
    private void setVariant(WaferWitchVariant variant) {
