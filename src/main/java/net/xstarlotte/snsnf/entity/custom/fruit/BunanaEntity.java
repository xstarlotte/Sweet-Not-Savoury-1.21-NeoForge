...
public class BunanaEntity extends TamableAnimal implements GeoEntity {

    private static final EntityDataAccessor<Boolean> SITTING =
            SynchedEntityData.defineId(BunanaEntity.class, EntityDataSerializers.BOOLEAN);

    public BunanaEntity(EntityType<? extends BunanaEntity> type, Level level) {
        super(type, level);
    ...
    private <T extends GeoAnimatable> PlayState predicate(AnimationState<T> tAnimationState) {
        if(tAnimationState.isMoving()) {
            tAnimationState.getController().setAnimation(RawAnimation.begin().then("animation.bunana.run", Animation.LoopType.LOOP));
            return PlayState.CONTINUE;
        }
        if(this.isSitting()) {
            tAnimationState.getController().setAnimation(RawAnimation.begin().then("animation.bunana.sit", Animation.LoopType.LOOP));
            return PlayState.CONTINUE;
        }
        tAnimationState.getController().setAnimation(RawAnimation.begin().then("animation.bunana.idle", Animation.LoopType.LOOP));
        return PlayState.CONTINUE;
    }
    @Override
...
//AI
    ...
    public AgeableMob getBreedOffspring(ServerLevel level, AgeableMob otherParent) {
        return SNSEntity.BUNANA.get().create(level);
    }
    ...
    public InteractionResult mobInteract(Player pPlayer, InteractionHand pHand) {
        ...
        Item itemForTaming = SNSItem.GUMMY_BANANA.get();
