...
public class CheesecakeMouseEntity extends TamableAnimal implements GeoEntity {

    private static final EntityDataAccessor<Integer> VARIANT =
            SynchedEntityData.defineId(CheesecakeMouseEntity.class, EntityDataSerializers.INT);
    private static final EntityDataAccessor<Boolean> SITTING =
            SynchedEntityData.defineId(CheesecakeMouseEntity.class, EntityDataSerializers.BOOLEAN);

    //animations

    public CheesecakeMouseEntity(EntityType<? extends CheesecakeMouseEntity> type, Level level) {
        super(type, level);
    }
    @Override
    ...
    private <T extends GeoAnimatable> PlayState predicate(AnimationState<T> tAnimationState) {
        if(tAnimationState.isMoving()) {
            tAnimationState.getController().setAnimation(RawAnimation.begin().then("animation.mouse.run", Animation.LoopType.LOOP));
            return PlayState.CONTINUE;
        }
        if(this.isSitting()) {
            tAnimationState.getController().setAnimation(RawAnimation.begin().then("animation.mouse.sit", Animation.LoopType.LOOP));
            return PlayState.CONTINUE;
        }
    ...
    public AgeableMob getBreedOffspring(ServerLevel level, AgeableMob otherParent) {
        return SNSEntity.CHEESECAKE_MOUSE.get().create(level);
    }
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor pLevel, DifficultyInstance pDifficulty, MobSpawnType pReason,
        ...
        CheesecakeMouseVariant variant = Util.getRandom(CheesecakeMouseVariant.values(), this.random);
    ...
    public CheesecakeMouseVariant getVariant() {
        return CheesecakeMouseVariant.byId(this.getTypeVariant() & 255);
    }
    private void setVariant(CheesecakeMouseVariant variant) {
