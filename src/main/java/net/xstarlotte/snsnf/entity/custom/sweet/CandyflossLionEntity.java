...
public class CandyflossLionEntity extends TamableAnimal implements GeoEntity {

    private static final EntityDataAccessor<Integer> VARIANT =
            SynchedEntityData.defineId(CandyflossLionEntity.class, EntityDataSerializers.INT);
    private static final EntityDataAccessor<Boolean> SITTING =
            SynchedEntityData.defineId(CandyflossLionEntity.class, EntityDataSerializers.BOOLEAN);

    //animations

    public CandyflossLionEntity(EntityType<? extends CandyflossLionEntity> type, Level level) {
        super(type, level);
    }
    @Override
    ...
    private <T extends GeoAnimatable> PlayState predicate(AnimationState<T> tAnimationState) {
        if(tAnimationState.isMoving()) {
            tAnimationState.getController().setAnimation(RawAnimation.begin().then("animation.lion.run", Animation.LoopType.LOOP));
            return PlayState.CONTINUE;
        }
        if(this.isSitting()) {
            tAnimationState.getController().setAnimation(RawAnimation.begin().then("animation.lion.sit", Animation.LoopType.LOOP));
            return PlayState.CONTINUE;
        }
        tAnimationState.getController().setAnimation(RawAnimation.begin().then("animation.lion.idle", Animation.LoopType.LOOP));
        return PlayState.CONTINUE;
    }
    @Override
    ...
    public AgeableMob getBreedOffspring(ServerLevel level, AgeableMob otherParent) {
        return SNSEntity.CANDYFLOSS_LION.get().create(level);
    }
    ...
    public InteractionResult mobInteract(Player pPlayer, InteractionHand pHand) {
        ...
        Item itemForTaming = SNSItem.CANDYFLOSS.get();
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor pLevel, DifficultyInstance pDifficulty, MobSpawnType pReason,
        ...
        CandyflossLionVariant variant = Util.getRandom(CandyflossLionVariant.values(), this.random);
    ...
    public CandyflossLionVariant getVariant() {
        return CandyflossLionVariant.byId(this.getTypeVariant() & 255);
    }
    private void setVariant(CandyflossLionVariant variant) {
