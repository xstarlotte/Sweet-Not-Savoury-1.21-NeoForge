...
public class PeppermintTwirlEntity extends TamableAnimal implements GeoEntity {

    private static final EntityDataAccessor<Boolean> SITTING =
            SynchedEntityData.defineId(PeppermintTwirlEntity.class, EntityDataSerializers.BOOLEAN);

    public PeppermintTwirlEntity(EntityType<? extends PeppermintTwirlEntity> type, Level level) {
        super(type, level);
    ...
    private <T extends GeoAnimatable> PlayState predicate(AnimationState<T> tAnimationState) {
        if(tAnimationState.isMoving()) {
            tAnimationState.getController().setAnimation(RawAnimation.begin().then("animation.peppermint_twirl.run", Animation.LoopType.LOOP));
            return PlayState.CONTINUE;
        }
        if(this.isSitting()) {
            tAnimationState.getController().setAnimation(RawAnimation.begin().then("animation.peppermint_twirl.sit", Animation.LoopType.LOOP));
            return PlayState.CONTINUE;
        }
        tAnimationState.getController().setAnimation(RawAnimation.begin().then("animation.peppermint_twirl.idle", Animation.LoopType.LOOP));
        return PlayState.CONTINUE;
    }
    @Override
...
//AI
    ...
    public AgeableMob getBreedOffspring(ServerLevel level, AgeableMob otherParent) {
        return SNSEntity.PEPPERMINT_TWIRL.get().create(level);
    }
