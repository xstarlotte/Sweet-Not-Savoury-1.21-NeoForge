...
public class BlackboarryEntity extends Animal implements GeoEntity {

    public BlackboarryEntity(EntityType<? extends BlackboarryEntity> type, Level level) {
        super(type, level);
    }
    ...
    private <T extends GeoAnimatable> PlayState predicate(AnimationState<T> tAnimationState) {
        if(tAnimationState.isMoving()) {
            tAnimationState.getController().setAnimation(RawAnimation.begin().then("animation.blackboarry.run", Animation.LoopType.LOOP));
            return PlayState.CONTINUE;
        }
        tAnimationState.getController().setAnimation(RawAnimation.begin().then("animation.blackboarry.idle", Animation.LoopType.LOOP));
        return PlayState.CONTINUE;
    }
    @Override
...
//AI
    ...
    public AgeableMob getBreedOffspring(ServerLevel level, AgeableMob otherParent) {
        return SNSEntity.BLACKBOARRY.get().create(level);
    }
    ...
    public void aiStep() {
        ...
        if (!this.level().isClientSide && this.isAlive() && !this.isBaby() && --this.eggTime <= 0) {
            ...
            this.spawnAtLocation(SNSItem.GUMMY_BLACKBERRY);
