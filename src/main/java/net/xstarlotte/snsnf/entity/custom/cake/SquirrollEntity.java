...
public class SquirrollEntity extends TamableAnimal implements GeoEntity {

    private static final EntityDataAccessor<Integer> VARIANT =
            SynchedEntityData.defineId(SquirrollEntity.class, EntityDataSerializers.INT);
    private static final EntityDataAccessor<Boolean> SITTING =
            SynchedEntityData.defineId(SquirrollEntity.class, EntityDataSerializers.BOOLEAN);

    //animations

    public SquirrollEntity(EntityType<? extends SquirrollEntity> type, Level level) {
        super(type, level);
    }
    @Override
    ...
    private <T extends GeoAnimatable> PlayState predicate(AnimationState<T> tAnimationState) {
        if(tAnimationState.isMoving()) {
            tAnimationState.getController().setAnimation(RawAnimation.begin().then("animation.squirroll.run", Animation.LoopType.LOOP));
            return PlayState.CONTINUE;
        }
        if(this.isSitting()) {
            tAnimationState.getController().setAnimation(RawAnimation.begin().then("animation.squirroll.sit", Animation.LoopType.LOOP));
            return PlayState.CONTINUE;
        }
        tAnimationState.getController().setAnimation(RawAnimation.begin().then("animation.squirroll.idle", Animation.LoopType.LOOP));
        return PlayState.CONTINUE;
    }
    @Override
    ...
    public AgeableMob getBreedOffspring(ServerLevel level, AgeableMob otherParent) {
        return SNSEntity.SQUIRROLL.get().create(level);
    }
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor pLevel, DifficultyInstance pDifficulty, MobSpawnType pReason,
        ...
        SquirrollVariant variant = Util.getRandom(SquirrollVariant.values(), this.random);
    ...
    public SquirrollVariant getVariant() {
        return SquirrollVariant.byId(this.getTypeVariant() & 255);
    }
    private void setVariant(SquirrollVariant variant) {
