...
public class ChocolateChickenEntity extends Animal implements GeoEntity {

    private static final EntityDataAccessor<Integer> VARIANT =
            SynchedEntityData.defineId(ChocolateChickenEntity.class, EntityDataSerializers.INT);
    
    public ChocolateChickenEntity(EntityType<? extends ChocolateChickenEntity> type, Level level) {
        super(type, level);
    }
    ...
    private <T extends GeoAnimatable> PlayState predicate(AnimationState<T> tAnimationState) {
        if(tAnimationState.isMoving()) {
            tAnimationState.getController().setAnimation(RawAnimation.begin().then("animation.chocolate_chicken.run", Animation.LoopType.LOOP));
            return PlayState.CONTINUE;
        }
        tAnimationState.getController().setAnimation(RawAnimation.begin().then("animation.chocolate_chicken.idle", Animation.LoopType.LOOP));
        return PlayState.CONTINUE;
    }
    @Override
...
//AI
    ...
    public AgeableMob getBreedOffspring(ServerLevel level, AgeableMob otherParent) {
        return SNSEntity.CHOCOLATE_CHICKEN.get().create(level);
    }
    ...
    public void aiStep() {
        ...
        if (!this.level().isClientSide && this.isAlive() && !this.isBaby() && --this.eggTime <= 0) {
            ...
            this.spawnAtLocation(SNSItem.CHOCOLATE_EGG);
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor pLevel, DifficultyInstance pDifficulty, MobSpawnType pReason,
        ...
        ChocolateChickenVariant variant = Util.getRandom(ChocolateChickenVariant.values(), this.random);
    ...
    public ChocolateChickenVariant getVariant() {
        return ChocolateChickenVariant.byId(this.getTypeVariant() & 255);
    }
    private void setVariant(ChocolateChickenVariant variant) {
