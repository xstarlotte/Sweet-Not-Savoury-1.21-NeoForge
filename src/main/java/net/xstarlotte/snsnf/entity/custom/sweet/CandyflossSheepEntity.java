...
public class CandyflossSheepEntity extends Animal implements GeoEntity {

    private static final EntityDataAccessor<Integer> VARIANT =
            SynchedEntityData.defineId(CandyflossSheepEntity.class, EntityDataSerializers.INT);

    public CandyflossSheepEntity(EntityType<? extends CandyflossSheepEntity> type, Level level) {
        super(type, level);
    }
    ...
    private <T extends GeoAnimatable> PlayState predicate(AnimationState<T> tAnimationState) {
        if(tAnimationState.isMoving()) {
            tAnimationState.getController().setAnimation(RawAnimation.begin().then("animation.candyfloss_sheep.run", Animation.LoopType.LOOP));
            return PlayState.CONTINUE;
        }
        tAnimationState.getController().setAnimation(RawAnimation.begin().then("animation.candyfloss_sheep.idle", Animation.LoopType.LOOP));
        return PlayState.CONTINUE;
    }
    @Override
...
//AI
    ...
    public AgeableMob getBreedOffspring(ServerLevel level, AgeableMob otherParent) {
        return SNSEntity.CANDYFLOSS_SHEEP.get().create(level);
    }
    ...
    public void aiStep() {
        ...
        if (!this.level().isClientSide && this.isAlive() && !this.isBaby() && --this.eggTime <= 0) {
            ...
            this.spawnAtLocation(SNSItem.CANDYFLOSS);
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor pLevel, DifficultyInstance pDifficulty, MobSpawnType pReason,
        ...
        CandyflossSheepVariant variant = Util.getRandom(CandyflossSheepVariant.values(), this.random);
    ...
    public CandyflossSheepVariant getVariant() {
        return CandyflossSheepVariant.byId(this.getTypeVariant() & 255);
    }
    private void setVariant(CandyflossSheepVariant variant) {
