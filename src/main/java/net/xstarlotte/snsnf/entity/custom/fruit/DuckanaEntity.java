...
public class DuckanaEntity extends Animal implements GeoEntity {

    public DuckanaEntity(EntityType<? extends DuckanaEntity> type, Level level) {
        super(type, level);
    }
    ...
    private <T extends GeoAnimatable> PlayState predicate(AnimationState<T> tAnimationState) {
        if(tAnimationState.isMoving()) {
            tAnimationState.getController().setAnimation(RawAnimation.begin().then("animation.duckana.run", Animation.LoopType.LOOP));
            return PlayState.CONTINUE;
        }
        tAnimationState.getController().setAnimation(RawAnimation.begin().then("animation.duckana.idle", Animation.LoopType.LOOP));
        return PlayState.CONTINUE;
    }
    @Override
...
//AI
    ...
    public AgeableMob getBreedOffspring(ServerLevel level, AgeableMob otherParent) {
        return SNSEntity.DUCKANA.get().create(level);
    }
    ...
    public void aiStep() {
        ...
        if (!this.level().isClientSide && this.isAlive() && !this.isBaby() && --this.eggTime <= 0) {
            ...
            this.spawnAtLocation(SNSItem.GUMMY_BANANA);
